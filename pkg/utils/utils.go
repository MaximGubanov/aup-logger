...
func CheckingFileExistence(logPath, filename string) (*os.File, error) {
	...
	if isMoreThan100MB(logFileName) {
		...
		file, err = os.OpenFile(logFileName, os.O_CREATE|os.O_WRONLY|os.O_APPEND|os.O_SYNC, 0777)
		if err == nil {
			writeLogHeader(file, filename)
		}
	} else {
		file, err = os.OpenFile(logFileName, os.O_WRONLY|os.O_APPEND|os.O_SYNC, 0777)
		if err != nil {
